...
#   Date:  Jan/24          #
############################
# github keychain:  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
...
################################################################################################
# Definindo lista com 24 cores para os Operadores e
...
def busca_horas_turno(dia_semana, hora_dia, minuto):
    if dia_semana in ('Sab', 'Sat') and hora_dia in ('05', '06', '07', '08'):
        hora_entrada = '05:00'
        ...
        hora_saida = [13, 0]  # às 13:00 (Hora e Minutos como inteiros)
            ...
            hora_dia in ('05', '06', '07', '08', '09', '10', '11', '12'):
        hora_entrada = '05:00'
...
def ajusta_dataframe():
    ...
    df_por_dia = df_filtrado.groupby('Dia')  # criando um dataframe agrupado por DIA e
    # print('\n\n========== AJUSTANDO O DATAFRAME ==========')
    for chave, reg in df_por_dia:
        # print('[][][][][] Dia (key):', chave)
        novo_dia = True
        registro_anterior = []
        for index, row in reg.iterrows():
            ...
            if novo_dia:  # o dia é novo ou mudou e precisa começar com uma ENTRADA
                ...
                if row['Evento'] == 'ENTRADA':
                    ...
                    # print('   Primeiro EVENTO desse dia não começa como ENTRADA. Descartandooooooooo...')
                    pass
                ...
                if row['Evento'] == 'ENTRADA' and registro_anterior[1] == 'SAIDA':
                    ...
                    if row['Nome'] == registro_anterior[2]:  # se for o mesmo Operador
                        ...
                        novo_registro[0] = registro[0]  # Hora da saida como sendo a da Entrada do corrente Operador
                        novo_registro[1] = 'SAIDA'  # para criar um evento de SAIDA
                        novo_registro[7] = registro[7]  # Hora da saida como sendo a da Entrada do corrente Operador
                        novo_registro[8] = registro[8]  # Minuto da saida como sendo a da Entrada do corrente Operador
                        ...
                        novo_registro[3] = registro_anterior[3]  # Matricula do Operador
                        novo_registro[7] = registro[7]  # Hora da saida como sendo a da Entrada do corrente Operador
                        novo_registro[8] = registro[8]  # Minuto da saida como sendo a da Entrada do corrente Operador
                    ...
                    if row['Nome'] == registro_anterior[2]:  # se for o mesmo Operador
                        # print('   > S/E do mesmo Operador - copiando o registro corrente')
                        df_new.loc[len(df_new)] = registro
                        registro_anterior = registro
                    else:  # se não for o mesmo Operador
                        # print('   > S/E por um Operador diferente, passando este evento para o Operador anterior')
                        registro_temp = row.tolist()
                        novo_registro = row.tolist()
                        novo_registro[2] = registro_anterior[2]  # Nome do Operador
                        novo_registro[3] = registro_anterior[3]  # Matricula do Operador
                        novo_registro[9] = registro_anterior[9]  # Cor associada
                        df_new.loc[len(df_new)] = novo_registro  # grava novo registro com dados do Operador de antes
                        registro_temp[1] = 'ENTRADA'
                        df_new.loc[len(df_new)] = registro_temp  # grava o registro do Operador atual como ENTRADA
                        registro_anterior = registro_temp

                elif row['Evento'] == 'SAIDA' and registro_anterior[1] == 'SAIDA':
                    if row['Nome'] == registro_anterior[2]:  # se for o mesmo Operador
                        # print('   > S/S pelo mesmo Operador -> Adicionando ENTRADA e SAIDA para Operador anterior')
                        novo_registro = registro_anterior
                        novo_registro[2] = 'ENTRADA'  # para criar um evento de ENTRADA
                        df_new.loc[len(df_new)] = novo_registro  # grava novo registro com dados do Operador de antes
                        df_new.loc[len(df_new)] = registro  # grava o registro do Operador atual
                        registro_anterior = registro
                    else:  # se não for o mesmo Operador
                        ...
                        novo_registro = row.tolist()
                        novo_registro[0] = registro_anterior[0]  # Hora da saida como sendo a da Entrada do corrente Operador
                        novo_registro[1] = 'ENTRADA'  # para criar um evento de ENTRADA
                        novo_registro[2] = registro[2]  # Nome do Operador
                        novo_registro[3] = registro[3]  # Matricula do Operador
                        novo_registro[7] = registro_anterior[7]  # Hora da saida como sendo a da Entrada do corrente Operador
                        novo_registro[8] = registro_anterior[8]  # Minuto da saida como sendo a da Entrada do corrente Operador
                        novo_registro[9] = registro[9]  # Cor associada
                        df_new.loc[len(df_new)] = novo_registro  # grava novo registro com dados do Operador de antes
                        df_new.loc[len(df_new)] = registro  # grava o registro do Operador atual
                        registro_anterior = registro

        if row['Evento'] == 'ENTRADA' or registro_anterior[1] == 'ENTRADA':  # se ultimo registro foi ENTRADA então cria uma SAIDA no final do turno
...
if uploaded_file is not None:
    ...
    if arq_tipo == 'csv_com_header':
        ...
        if not df.empty:
            ...
            df = df.drop_duplicates(subset=['Data', 'Nome'], keep=False)  # Retirando as linhas duplicadas
            df = df.apply(
            ...
            if not df_entradas.empty:
                ...
                st.divider()
                st.markdown('#### Total de Horas por Operador no periodo selecionado:')
                st.markdown(f'##### Percentual trabalhado com base nas {horas_disponiveis} horas disponiveis no período')
                st.markdown(
                    '''
                            <style>
                ...
                for i, vcol in enumerate(col_list):
                    ...
                    vcol.metric(label=lista_chaves[i],
                                value=('{:,.3}h'.format(total_trabalhado)),
                ...
                col1.dataframe(df_entradas.style.set_properties(**properties), hide_index=True,
                               ...
                               column_config={'AnoMes': None, 'Cor': None, 'RegistrouSaida': None, 'Dia': None, 'Hora': None, 'Minuto': None})
